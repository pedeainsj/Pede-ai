...
// no mesmo contexto JS, então toda leitura aqui já assume um db potencialmente
// recém-recriado em config.js após reload.
...
function renderizarMediaCard(produto, modo) {
    if (produto.videoUrl && produto.videoUrl.trim() !== "") {
        ...
        return `<img src="${imgOptimized}" loading="lazy" crossorigin="anonymous" style="width:100%;height:100%;background:#fcfcfc;display:block;transition:opacity 0.2s;object-fit:cover;object-position:center center;" onerror="this.removeAttribute('crossorigin');this.onerror=null;this.src=this.src;" onload="(function(img){try{const ratio=img.naturalWidth/img.naturalHeight;if(ratio>0.85){img.style.objectFit='contain';img.style.objectPosition='center center';return;}const W=60,H=80,S=8;const c=document.createElement('canvas');c.width=W;c.height=H;const ctx=c.getContext('2d');ctx.drawImage(img,0,0,W,H);const d=ctx.getImageData(0,0,W,H).data;const sliceScore=[];for(let s=0;s<S;s++){let nonBg=0;const y0=Math.floor(s*H/S),y1=Math.floor((s+1)*H/S);for(let y=y0;y<y1;y++){for(let x=0;x<W;x++){const i=(y*W+x)*4,r=d[i],g=d[i+1],b=d[i+2];const mx=Math.max(r,g,b),mn=Math.min(r,g,b);const isWhitish=(r>210&&g>210&&b>210);const isGrayish=(mx-mn<18&&r>160);const lum=0.299*r+0.587*g+0.114*b;const isBlackish=lum<30;if(!isWhitish&&!isGrayish&&!isBlackish)nonBg++;}}sliceScore.push(nonBg);}const total=sliceScore.reduce((a,b)=>a+b,0);if(total===0){img.style.objectPosition='center 20%';return;}let weightedY=0;for(let s=0;s<S;s++)weightedY+=sliceScore[s]*((s+0.5)/S);const centerFrac=weightedY/total;const pct=Math.round(Math.max(5,Math.min(80,centerFrac*100)));img.style.objectPosition='center '+pct+'%';}catch(e){img.style.objectPosition='center center';}})(this)">`;
    }
}
      ...
      try {
        ...
        if (isReturning) {
          ...
          if (domCache && grid && domCache.trim() !== "") {
            ...
            grid.querySelectorAll('img[crossorigin]').forEach(img => {
            ...
            try { inicializarArialProdutos(); } catch (e) { console.error('Falha ao reanimar carrossel (retorno):', e); }
