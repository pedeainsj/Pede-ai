...
function inicializarCarrinho() {
    ...
    // Substitui o polling fixo (setInterval sem cleanup) por um intervalo
    // que se desliga automaticamente quando a página sai de foco/é
    // descartada (pagehide) e quando volta ao primeiro plano via bfcache
    // (pageshow), evitando timers "pendurados" se o navegador congelar
    // a página em vez de destruí-la ao usar o botão voltar.
    let intervalIconeCarrinho = setInterval(window.atualizarIconeCarrinho, 400);

    window.addEventListener('pagehide', () => {
        clearInterval(intervalIconeCarrinho);
    });

    window.addEventListener('pageshow', (ev) => {
        if (ev.persisted) {
            clearInterval(intervalIconeCarrinho);
            intervalIconeCarrinho = setInterval(window.atualizarIconeCarrinho, 400);
            window.atualizarIconeCarrinho();
        }
    });
}
