...
// Fecha a conexão Firestore desta página ANTES de navegar de volta ao index,
// em vez de depender só do evento 'pagehide' (best-effort, não garantido no
// WKWebView do Capacitor). Mesmo princípio aplicado em navegarParaProduto no
// index.js: fechar o canal de forma síncrona com o clique, não na saída.
window.voltarParaIndex = async () => {
    // Não terminamos mais o canal a cada navegação — a instância do
    // Firestore agora é única por sessão do app (ver config.js), então não
    // há mais necessidade de fechar e reabrir o canal a cada ida e volta.
...
export function restaurarEstadoVisualVitrine() {
    ...
    const overlayEntrada = document.getElementById('overlayTransicaoEntrada');
    if (overlayEntrada) {
        overlayEntrada.style.opacity = '0';
        overlayEntrada.style.visibility = 'hidden';
    }
}
...
/* ============================================================================
 * PIPELINE DE RENDERIZAÇÃO DA VITRINE
 * ----------------------------------------------------------------------------
 * A partir daqui, o carregamento da vitrine é dividido em etapas isoladas e
 * sequenciais. Cada etapa só lê o que recebe como parâmetro — nenhuma etapa
 * depende de variável de módulo deixada por uma renderização anterior — e
 * cada etapa OU retorna o dado pronto OU lança um erro. Nenhuma etapa toca
 * no skeleton: a remoção do skeleton é responsabilidade exclusiva do
 * orquestrador (carregarVitrineCompleta), em exatamente um lugar de sucesso
 * e um lugar de erro. Isso elimina estruturalmente a possibilidade de um
 * caminho de código "esquecer" de remover o skeleton: não há mais nenhum
 * "return" no meio do fluxo que possa pular essa responsabilidade, porque
 * nenhuma etapa tem permissão para retornar silenciosamente — ela ou entrega
 * o resultado, ou lança, e quem decide o que fazer com isso é sempre o
 * mesmo lugar.
 * ==========================================================================*/

// ETAPA 1 — Leitura e validação dos parâmetros da URL. Função pura: não toca
// no DOM além de localizar o container, não depende de nada além da própria
// URL atual. Lança erro descritivo se o container não existir no documento.
function lerEValidarParametrosVitrine() {
    ...
    if (!mainContainer) {
        throw new Error('vitrine_sem_container');
    }

    return { sellerId, activeProductId, modo, mainContainer };
}

// ETAPA 2 — Carregamento da loja (lojista). Lança erros tipados em vez de
// retornar silenciosamente, para que o orquestrador trate cada caso de forma
// explícita (loja bloqueada, loja inexistente) sem nunca pular a resolução
// do skeleton. Quando não há sellerId na URL, devolve os valores neutros que
// o restante do pipeline já esperava nesse caso.
async function carregarLoja(sellerId) {
    if (!sellerId) {
        return {
            lojistaInfo: { nomeLoja: "Loja", fotoPerfil: "" },
            regrasLojista: { podeExibirProdutos: true }
        };
    }
    ...
    if (!s.exists()) {
        const erro = new Error('vitrine_lojista_inexistente');
        erro.tipo = 'lojista_inexistente';
        throw erro;
    }

    const lojistaInfo = s.data();
    lojistaInfoCache = lojistaInfo;
    lojistaInfoCache.id = sellerId;
    const regrasLojista = GetRegrasLojista(lojistaInfo);

    if (!regrasLojista.podeExibirProdutos || regrasLojista.isBloqueado) {
        const erro = new Error('vitrine_lojista_bloqueado');
        erro.tipo = 'lojista_bloqueado';
        throw erro;
    }

    return { lojistaInfo, regrasLojista };
}

// Constrói o HTML do cabeçalho da loja — separado de carregarLoja() para que
// a etapa de leitura de dados (assíncrona, pode falhar) não fique misturada
// com a etapa de montagem de HTML (síncrona, sempre sucede).
function construirHeaderLoja(sellerId, modo, lojistaInfo) {
...
<div style="display: flex; align-items: center; width: 100%; justify-content: space-between; padding: 0 5px;">
    ...
    <div style="display: flex; align-items: center; min-width: 0;">
        <a href="javascript:void(0)" class="back-btn" onclick="window.voltarParaIndex(); return false;">
...
// ETAPA 3 — Carregamento dos produtos (catálogo completo + produto ativo).
// Mantém exatamente a mesma estratégia de timeout + uma única nova tentativa
// que já existia, apenas isolada em função própria e sem nenhum "return"
// silencioso: ou devolve {snap, docPrincipal}, ou deixa o erro propagar.
async function carregarProdutos(activeProductId) {
    const _firestoreTimeout = new Promise((_, rej) =>
        setTimeout(() => rej(new Error('vitrine_timeout')), 22000)
    );
...
// ETAPA 4 — Construção do HTML em memória. Função pura e síncrona: recebe os
// dados já carregados e devolve strings prontas, sem tocar no DOM. Mantém
// literalmente a mesma lógica de montagem de card "destaque" (gourmet,
// produto normal, produto roupa) e da grade "mais desta loja" que já existia
// — apenas isolada do carregamento de dados e da escrita no DOM, que agora
// são responsabilidade de outras etapas.
function construirHTMLProdutos(snap, docPrincipal, sellerId, activeProductId, modo) {
        let htmlDestaque = "";
        let htmlGridLojista = "";
        let categoriaAtiva = "";
...
const funcAddConfig = adicionaisProduto.length > 0 
    ...
    : `window.adicionarAoCarrinho('${d.id}', '${nomeSanitizado}', '${p.preco}', '${p.owner}', '${p.whatsapp}', '${imgCapaRaw}', '${linkProduto}', '${descSanitizada}')`;
            ...
            // CORREÇÃO CRÍTICA: o produto ativo (activeProductId, vindo da URL)
            // NUNCA pode ser excluído por este filtro de categoria, mesmo que
            // p.categoria e categoriaAtiva estejam temporariamente divergentes
            // (ex.: docPrincipal e snap chegando de momentos/estados de cache
            // ligeiramente diferentes do SDK do Firestore sob concorrência —
            // mais frequente no WKWebView/Safari do que no Chrome/Android).
            // Antes, quando essa divergência acontecia, "return" pulava o
            // produto ativo inteiro ANTES da linha que preenche
            // imagemCapaProdutoAtivo (abaixo) — então htmlDestaque ficava
            // vazio, o skeleton saía normalmente (aplicarRenderizacao
            // completa sem erro) e só a grade "Mais desta loja" aparecia, sem
            // nenhuma imagem ou produto principal — exatamente o sintoma de
            // "skeleton some, mas a imagem não renderiza", sem nenhum erro de
            // rede de fato. O filtro de categoria continua válido para todos
            // os OUTROS produtos da grade — só o produto que o usuário
            // explicitamente pediu para ver nunca pode ser descartado por ele.
            if (d.id !== activeProductId && p.categoria !== categoriaAtiva) return;
...
// Adiciona ícone de vídeo se existir
        ...
        return { htmlDestaque, htmlGridLojista, imagemCapaProdutoAtivo };
}
// ETAPA 5 — Confirmação de renderização e aplicação ao DOM. Esta é a ÚNICA
// etapa que escreve em mainContainer.innerHTML. Mantém literalmente a mesma
// lógica de pré-carregamento de imagem de capa e de conexão dos listeners de
// slider/vídeo que já existia. Só retorna depois que o DOM já foi
// efetivamente construído e a imagem de capa já carregou (ou expirou) — ou
// seja, o retorno desta função É a "confirmação de renderização" pedida: o
// orquestrador só remove o skeleton depois que esta função termina.
async function aplicarRenderizacao(mainContainer, htmlDestaque, htmlGridLojista, lojistaInfo, imagemCapaProdutoAtivo) {
...
export async function carregarVitrineCompleta() {
    // Trava de concorrência: impede que uma segunda chamada (disparada pelo
    // listener de pageshow enquanto a primeira ainda está em andamento)
    // sobrescreva o DOM no meio de uma renderização e deixe o skeleton preso
    // sem nunca alcançar o catch externo.
    if (carregamentoVitrineEmAndamento) return;
    carregamentoVitrineEmAndamento = true;


    // ORQUESTRADOR — único responsável por decidir quando o skeleton sai da
    // tela. Cada etapa do pipeline (lerEValidarParametrosVitrine, carregarLoja,
    // carregarProdutos, construirHTMLProdutos, aplicarRenderizacao) ou entrega
    // o resultado ou lança um erro: não existe mais nenhum "return" no meio do
    // caminho que possa pular a resolução do skeleton. Por isso o skeleton só
    // é tratado em exatamente dois lugares neste arquivo inteiro: aqui embaixo,
    // no bloco de sucesso (depois que aplicarRenderizacao confirma que o DOM
    // já foi construído), e no catch, para qualquer etapa que tenha falhado.
    let mainContainer;
    try {
        const parametros = lerEValidarParametrosVitrine();
        mainContainer = parametros.mainContainer;
        const { sellerId, activeProductId, modo } = parametros;

        const _registrarRedesDeSeguranca = _criarRedesDeSegurancaSkeleton(mainContainer);

        try {
            const { lojistaInfo, regrasLojista } = await carregarLoja(sellerId);

            if (sellerId) {
                const header = document.getElementById('main-header');
                if (header) {
                    header.innerHTML = construirHeaderLoja(sellerId, modo, lojistaInfo);
                }
            }

            const { snap, docPrincipal } = await carregarProdutos(activeProductId);

            const { htmlDestaque, htmlGridLojista, imagemCapaProdutoAtivo } =
                construirHTMLProdutos(snap, docPrincipal, sellerId, activeProductId, modo);

            // Confirmação de renderização: só depois que o DOM foi efetivamente
            // construído e a imagem de capa já carregou (ou expirou) é que
            // consideramos a vitrine "pronta" — e só então o skeleton sai.
            await aplicarRenderizacao(mainContainer, htmlDestaque, htmlGridLojista, lojistaInfo, imagemCapaProdutoAtivo);

            esconderSkeletonVitrine();
        } finally {
            _registrarRedesDeSeguranca.desarmar();
        }

    } catch (error) {
        // Cobre TODOS os casos de falha do pipeline: lojista bloqueado, lojista
        // inexistente, timeout do Firestore, ou qualquer outro erro inesperado.
        // Como nenhuma etapa acima faz "return" sem lançar, este é o único
        // outro lugar (além do sucesso acima) que decide o destino do skeleton
        // — garantia estrutural de que ele nunca fica órfão.
        console.error("Erro ao carregar vitrine:", error);
        esconderSkeletonVitrine();

        if (mainContainer) {
            if (error && error.tipo === 'lojista_bloqueado') {
                // Mesmo comportamento visual que já existia: loja bloqueada
                // simplesmente não mostra produto, sem tela de erro.
                mainContainer.innerHTML = "";
            } else if (mainContainer.innerHTML.trim() === '') {
                // Antes, ao cair aqui o #productDetail ficava vazio e cinza para
                // sempre, sem nenhuma indicação visual de erro nem forma de
                // tentar de novo — sintoma de "produto não renderiza" após
                // muitas navegações, quando a conexão de rede do Firestore está
                // saturada. Agora mostramos um estado de erro com botão de
                // retry manual, em vez de tela cinza sem explicação.
                mainContainer.style.display = 'block';
                mainContainer.innerHTML = `
        ...
        carregamentoVitrineEmAndamento = false;
    }
}

// Centraliza as duas redes de segurança independentes (teto de tempo absoluto
// e verificação ao reganhar foco) num único lugar, retornando uma forma de
// desarmá-las assim que o pipeline principal terminar — sucesso ou erro. Isso
// evita duplicar a lógica de cada rede de segurança em dois pontos do código,
// como acontecia antes.
function _criarRedesDeSegurancaSkeleton(mainContainer) {
    // Marca o instante em que o pipeline desta vitrine começou, para que o
    // listener de 'visibilitychange' abaixo nunca possa aplicar a tela de
    // erro antes de um piso mínimo de tempo ter realmente passado.
    const _inicioCarregamento = Date.now();
    const _PISO_MINIMO_MS = 10000;

    function _aplicarTelaErroSeAindaPreso() {
        if (mainContainer.innerHTML.trim() === '' || mainContainer.style.display === 'none') {
            esconderSkeletonVitrine();
            mainContainer.style.display = 'block';
            mainContainer.innerHTML = `
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:64px 24px; text-align:center; min-height:300px;">
    ...
    // Teto de segurança real: se nenhum dos timeouts internos disparar (Promise
    // original "perdida" pelo Promise.race que nunca resolve nem rejeita —
    // caso de canal Firestore zumbi após terminate() disparado em sequência
    // rápida de navegações), isso garante que a tela de erro com retry sempre
    // apareça em vez do skeleton ficar preso para sempre.
    // CORREÇÃO CRÍTICA: 26000ms estava perigosamente colado no teto máximo
    // que o próprio carregarProdutos() já reserva para si mesmo (10s da
    // primeira tentativa + 15s do retry = 25s), sem nenhuma margem real para
    // a etapa de construção de HTML, pré-carregamento de imagem (até 4s) e
    // aplicação no DOM que vêm depois. No Safari/WKWebView, onde a primeira
    // conexão do WebChannel do Firestore é mais lenta que reconexões
    // subsequentes, esse teto de 1s de margem podia disparar a tela de erro
    // exatamente no meio do retry que estava prestes a funcionar — daí o
    // "Recarregar" sempre resolver na hora, não por falta de internet, mas
    // porque a segunda carga reaproveita uma conexão já estabelecida.
    // Reajustado para dar margem real ao novo teto de 22s da 1ª tentativa +
    // 15s do retry = 37s no pior caso, mais a etapa de construção de HTML e
    // pré-carregamento de imagem (até 4s) que vêm depois.
    const _tetoAbsoluto = setTimeout(_aplicarTelaErroSeAindaPreso, 42000);

    // Rede de segurança adicional: o setTimeout acima depende do timer
    // sobreviver ininterrupto por 26s, mas o WKWebView do Capacitor pode
    // suspender/atrasar timers de JS durante transições de navegação. O
    // evento 'visibilitychange' é garantido pelo WebKit mesmo quando
    // setTimeout é suspenso, então funciona como verificação independente.
    const _verificarAoRecuperarFoco = () => {
        if (document.visibilityState === 'visible' && carregamentoVitrineEmAndamento) {
            // GUARDA CRÍTICA: 'visibilitychange' no iOS dispara em situações
            // muito mais amplas do que "o usuário voltou de minimizar o app"
            // — inclui transições da própria navegação que trouxe esta
            // página, gestos de swipe da barra do Safari, e variações de
            // foco do teclado. Sem este piso mínimo, o evento podia disparar
            // a tela de erro nos primeiros segundos do load, quando
            // mainContainer ainda estar vazio é normal e esperado — ignorando
            // por completo o timeout de 34s configurado acima. Isso explicava
            // por que aumentar o valor do _tetoAbsoluto não tinha efeito
            // nenhum: este listener cortava o processo por um caminho que
            // nunca respeitava aquele número.
            if (Date.now() - _inicioCarregamento < _PISO_MINIMO_MS) return;
            _aplicarTelaErroSeAindaPreso();
        }
    };
    document.addEventListener('visibilitychange', _verificarAoRecuperarFoco);

    return {
        desarmar() {
            clearTimeout(_tetoAbsoluto);
            document.removeEventListener('visibilitychange', _verificarAoRecuperarFoco);
        }
    };
}
